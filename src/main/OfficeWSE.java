...
public class OfficeWSE {
	...
	public static void main(String[] args) {
		...
		OfficeWSE OS = new OfficeWSE();
		OS.Start_OfficeWSE();
	}
	
	public void Start_OfficeWSE() {
	...
	private boolean Start_Schedule_Listener() {
		// trigger: Job을 언제, 어떤 주기로, 언제부터 언제까지 실행할지에 대한 정보
	...
	private boolean Init_Configure() {
		...
		try {
			...
			if(!CONF.setParser("ini", "OfficeWSE")) {
        		ERR_MSG = "Init_Configure() Fail : Wrong ini file(OfficeWSE.ini)";
        		return false;
        	}
			...
			// JOB:ADD PATH
			String base_root_path	= CONF.getString("COMMON","BASE_ROOT_PATH","");
			String base_folder_path	= CONF.getString("COMMON","BASE_FOLDER_PATH","");
			...
			if(root_path.equals("") || index_path.equals("") || data_path.equals("") || base_root_path.equals("") || base_folder_path.equals("")) {
			...
			if(!Make_Dir(base_root_path+"\\"+base_folder_path)) {
				ERR_MSG = "Init_Configure() Fail : Can`t Create Folder ("+base_root_path+"\\"+base_folder_path+")";
        		return false;
			} else {
				
				try {
					OutputStream output = new FileOutputStream(base_root_path+"\\"+base_folder_path+"\\"+"Sample.txt");
				    String str ="x";
				...
				// File file = new File(base_root_path+"\\"+base_folder_path, "Sample.txt");
//				 if(file.exists()) {
...
//					 TRACE.TraceBox(file.getPath() + " : Successful file create.");
//				 } 
